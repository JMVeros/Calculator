...
const DealStructure = () => {
  ...
  const handleBlur = (key) => async () => {
    ...
    if (fields[key].value === originalValueRef.current) {
        return;
    }

    setFields((prev) => ({
      ...prev,
      [key]: { ...prev[key], isSaving: true, hasError: false },
    }));
    ...
    try {
      ...
      const numericValue = parseFloat(fields[key].value.replace(/[^0-9.]/g, ''));
      ...
      setFields((currentFields) => {
        // Recalculate and set the total after successful save
        setAmountFinanced(calculateTotal(currentFields));
        return {
          ...currentFields,
      ...
      setFields((prev) => ({
        ...prev,
        [key]: { ...prev[key], isSaving: false, hasError: true },
      }));
    }
  };
        ...
        <Card className="max-w-2xl w-full mx-auto">
            ...
            <form onSubmit={(e) => e.preventDefault()}>
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    <Field label="Sales Price" value={fields.salesPrice.value} onChange={handleFieldChange('salesPrice')} onFocus={handleFocus('salesPrice')} onBlur={handleBlur('salesPrice')} isSaving={fields.salesPrice.isSaving} hasError={fields.salesPrice.hasError} />
                    <Field label="Down Payment" value={fields.downPayment.value} onChange={handleFieldChange('downPayment')} onFocus={handleFocus('downPayment')} onBlur={handleBlur('downPayment')} isSaving={fields.downPayment.isSaving} hasError={fields.downPayment.hasError} />
                    <Field label="+ Warranty" value={fields.warranty.value} onChange={handleFieldChange('warranty')} onFocus={handleFocus('warranty')} onBlur={handleBlur('warranty')} isSaving={fields.warranty.isSaving} hasError={fields.warranty.hasError} />
                    <Field label="+ CPI" value={fields.cpi.value} onChange={handleFieldChange('cpi')} onFocus={handleFocus('cpi')} onBlur={handleBlur('cpi')} isSaving={fields.cpi.isSaving} hasError={fields.cpi.hasError} />
                    <Field label="+ Gap on Contract" value={fields.gap.value} onChange={handleFieldChange('gap')} onFocus={handleFocus('gap')} onBlur={handleBlur('gap')} isSaving={fields.gap.isSaving} hasError={fields.gap.hasError} />
                </CardContent>
            </form>
        </Card>
    </main>
